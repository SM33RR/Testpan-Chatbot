 ...
 */
  ...
  const HISTORY_KEY = 'testpan_chat_history';
  ...
  function createWidget(isIframe = false) {
    ...
    if (!isIframe) {
      ...
      launcher.innerHTML = `<img src="/manpowercustomer.png" alt="Support" style="width: 28px; height: 28px; border-radius: 50%;">`;
  ...
  function initializeSocket() {
    ...
    try {
      ...
      socket.on('bot_status_update', (data) => {
        if (data && data.text) {
          // Use a system message to show status without disrupting the message flow
  ...
  function showTypingIndicator() {
    ...
    typingDiv.innerHTML = `      <div class="testpan-message-bubble">
  ...
  function saveChatHistory() {
    try {
      // Limit history to the last 20 messages to avoid large localStorage usage
  ...
  function rehydrateChat() {
    try {
      ...
      console.error('Could not rehydrate chat from localStorage:', error);
