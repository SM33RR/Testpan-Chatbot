...
io.on('connection', (socket) => {
  ...
  processMessage(socket, 'menu', site).then(response => { if (response) socket.emit('bot_message', response); });
  ...
  socket.on('user_message', async (data) => {
    ...
    try {
      ...
      const response = await processMessage(socket, message, socket.data.currentSite);
      console.log(`[AI TIMING] "${message}" — processMessage() resolved in ${Date.now() - handlerStart}ms`);

      // If the response is not null (i.e., it wasn't a stream handled directly), send it.
      if (response) {
